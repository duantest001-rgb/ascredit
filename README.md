...
- AI Analyze button through Cloudflare Worker + Claude API
- Pipeline table
- Dashboard summary
...
## AI Analyze setup with Claude API
...
To make the AI Analyze button work:

1. Create or open your Cloudflare Worker.
2. Replace Worker code with `worker-ai-analyze.js` from this package.
3. In Worker Settings > Variables, add a Secret:
...
4. Deploy the Worker.
5. Deploy this web app to Cloudflare Pages again.
6. Open the app, fill a lead, then click `AI Analyze`.

Never place Claude API key in `js/config.js`, GitHub, or browser-side code.
